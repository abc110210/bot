...
static void ScanRecursive(const std::wstring& dirAbs,
    ...
    do {
        ...
        const std::wstring childRel = relInZip.empty() ? name : (relInZip + L"/" + name);

        if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            Entry e;
            e.fullPath  = childAbs;
            e.nameInZip = childRel + L"/";
            e.isDir     = true;
...
PackResult CreateEncryptedZip(const std::wstring& outPath,
    ...
    if (password.empty()) { res.error = L"压缩密码为空"; return res; }
    if (entries.empty())  { res.error = L"没有需要打包的内容"; return res; }
    if (entries.size() > 65000) {
        res.error = L"文件数量超过 65000，超出传统 ZIP 上限";
        return res;
    }
    ...
    FileWriter fw;
    if (!fw.Open(outPath)) {
        res.error = L"无法创建临时压缩包：" + util::LastErrorText(::GetLastError());
        return res;
    }
    ...
    for (const auto& e : entries) {
        if (!report(e.nameInZip)) { res.error = L"已取消"; return res; }
        ...
        if (cr.localOffset > 0xFFFFFFF0ull) {
            res.error = L"压缩包超过 4GB，传统 ZIP 无法承载，请减少打包内容";
            return res;
        }
        ...
        if (e.isDir) {
            ...
            if (!fw.Write(header)) { res.error = L"写入压缩包失败"; return res; }
        ...
        if (e.size > 0xFFFFFFF0ull) {
            res.error = L"存在超过 4GB 的单个文件，无法打包：" + e.nameInZip;
            return res;
        }
        ...
        if (!useStreaming) {
            ...
            if (!ComputeCrcStreaming(e.fullPath, crc, realSize, cancel)) {
                if (cancel && cancel->load()) { res.error = L"已取消"; return res; }
                res.skipped++;
                continue;
            }
        ...
        if (!fw.Write(encHeader, 12)) { res.error = L"写入压缩包失败"; return res; }

        if (!useStreaming) {
            if (!comp.empty()) {
                cipher.Encrypt(comp.data(), comp.size());
                if (!fw.Write(comp.data(), comp.size())) { res.error = L"写入压缩包失败"; return res; }
            }
            done += realSize;
        } else {
            HANDLE h = ::CreateFileW(e.fullPath.c_str(), GENERIC_READ,
                                     ...
                                     nullptr, OPEN_EXISTING,
                                     FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
            if (h == INVALID_HANDLE_VALUE) { res.error = L"读取文件失败：" + e.nameInZip; return res; }

            std::vector<uint8_t> buf(1u << 20);
            unsigned long long wrote = 0;
            bool ioOk = true;
            while (wrote < realSize) {
                if (!report(e.nameInZip)) { ::CloseHandle(h); res.error = L"已取消"; return res; }
                DWORD got = 0;
            ...
            ::CloseHandle(h);
            if (!ioOk || wrote != realSize) { res.error = L"读取文件中途失败：" + e.nameInZip; return res; }
        }
    ...
    if (!report(L"正在生成索引")) { res.error = L"已取消"; return res; }
    ...
    if (!fw.Write(cd)) { res.error = L"写入中央目录失败"; return res; }

    const unsigned long long cdSize = cd.size();
    if (cdOffset > 0xFFFFFFF0ull) {
        res.error = L"压缩包超过 4GB，无法生成有效索引";
        return res;
    }
    ...
    PutU16(eocd, 0);
    if (!fw.Write(eocd)) { res.error = L"写入结尾记录失败"; return res; }
