...
std::string Value::GetStr(const std::string& key, const std::string& def) const {
    ...
    if (v->type == Type::Number) {
        ...
        if (v->num == (double)(long long)v->num)
            ::snprintf(buf, sizeof(buf), OBFA(OBFA("Sld4c1pBPT0=")), (long long)v->num);
        else
            ::snprintf(buf, sizeof(buf), OBFA(OBFA("SldjPQ==")), v->num);
        return buf;
    }
    if (v->type == Type::Bool) return v->b ? OBFA(OBFA("ZEhKMVpRPT0=")) : OBFA(OBFA("Wm1Gc2MyVT0="));
    return def;
}
...
bool Value::GetBool(const std::string& key, bool def) const {
    ...
    if (v->type == Type::String) return v->str == OBFA(OBFA("ZEhKMVpRPT0=")) || v->str == OBFA(OBFA("TVE9PQ=="));
    return def;
}
...
public:
    ...
    ValuePtr ParseValue(int depth) {
        ...
        switch (c) {
            ...
            case 't':
                if (s_.compare(i_, 4, OBFA(OBFA("ZEhKMVpRPT0="))) == 0) {
                    i_ += 4;
                    auto v = std::make_shared<Value>();
                ...
                return nullptr;
            case 'f':
                if (s_.compare(i_, 5, OBFA(OBFA("Wm1Gc2MyVT0="))) == 0) {
                    i_ += 5;
                    auto v = std::make_shared<Value>();
                ...
                return nullptr;
            case 'n':
                if (s_.compare(i_, 4, OBFA(OBFA("Ym5Wc2JBPT0="))) == 0) {
                    i_ += 4;
...
std::string EscapeString(const std::string& s) {
    ...
    for (unsigned char c : s) {
        switch (c) {
            case '"':  out += OBFA(OBFA("WENJPQ==")); break;
            case '\\': out += OBFA(OBFA("WEZ3PQ==")); break;
            case '\b': out += OBFA(OBFA("WEdJPQ=="));  break;
            case '\f': out += OBFA(OBFA("WEdZPQ=="));  break;
            case '\n': out += OBFA(OBFA("WEc0PQ=="));  break;
            case '\r': out += OBFA(OBFA("WEhJPQ=="));  break;
            case '\t': out += OBFA(OBFA("WEhRPQ=="));  break;
            default:
                if (c < 0x20) {
                    char buf[8]{};
                    ::snprintf(buf, sizeof(buf), OBFA(OBFA("WEhVbE1EUjQ=")), c);
                    out += buf;
                } else {
